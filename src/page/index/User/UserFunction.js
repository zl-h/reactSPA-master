...
export function executeLanguage(data, index, action){
    ...
    return dispatch => {
        if(action.type === LanguageGetDataAction.type){
            //加载用户数据
            console.log("加载用户数据");
            dispatch(commonFetch(url.index_user_getTableData_url,action.requestBody,LanguageGetDataActionLoading,LanguageGetDataActionSuccess,LanguageGetDataActionFailure));
        }
        else if(action.type === LanguagedeleteDataAction.type){
            //加载用户数据
            console.log("删除语言数据");
            dispatch(commonFetch(url.index_user_removeData_url,action.requestBody,LanguagedeleteDataActionLoading,LanguagedeleteDataActionSuccess,LanguagedeleteDataActionFailure));
        }
            ...
            dispatch(commonFetch(url.index_user_editData_url,json,LanguageEditDataActionLoading,LanguageEditDataActionSuccess,LanguageEditDataActionFailure));
        }
            ...
            if(trim(item.userName.value) === "" || trim(item.trueName.value) === ""){
                openNotification("您有未填字段");
                return ;
            }else {
                 dispatch(commonFetch(url.index_user_addData_url,json,LanguageAddDataActionLoading,LanguageAddDataActionSuccess,LanguageAddDataActionFailure));
            }
        }
       /* else if(action.type === LanguagedeleteDataAction.type){
            //加载用户数据
            ...
            if(typeof json.ids === 'undefined' || json.ids.size === 0){
                ...
                confirm({
                    ...
                    content: '批量删除后不可恢复',
                    onOk() {
                        dispatch(commonFetch(url.index_user_removeData_url,json,LanguagedeleteDataActionLoading,LanguagedeleteDataActionSuccess,LanguagedeleteDataActionFailure));
